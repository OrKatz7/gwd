...
appContext = Context('configuration.cfg')
...
def warp(image_data,row,size=1024,teta=10,phi=10):
    image_data = cv2.resize(image_data,(size,size))
    bounding_boxes = []
    for f in row:
        bounding_boxes0 = {"cx":f[1],"cy":f[2],"w":f[3],"h":f[4]}
    ...
    bounding_boxes_result = np.array(bounding_boxes_result)
    image_data2 = cv2.resize(image_data2,(1024,1024))
    return image_data2,bounding_boxes_result
def warp_image_first(image_data,f,teta,phi):
    image_data1,bounding_boxes_result = warp(image_data,f,size=1024,teta=teta,phi=phi)
    bounding_boxes_result = np.array(bounding_boxes_result)
    return image_data1,bounding_boxes_result
...
def load_mosaic(self, index,mixup=False):
    ...
    for i, index in enumerate(indices):
        ...
        x = self.labels[index]
        labels = x.copy()
        #img, labels = warp_image_first(img,labels*1024,teta=50,phi=0)
        #img = img[:,:,0:3]
        # place img in img4
        if i == 0:  # top left
...
# def load_mixup2(self, index):
#     count = 1
#     s = self.img_size
#     if True:
#         labels4 = []
#         image, boxes = load_mosaic(self, index)
#         image = image.astype(np.float32)
#         image /= 255.0
#         new_index = random.randint(0, len(self.labels) - 1)
#         r_image,r_boxes load_mosaic(self, new_index)
#         r_image = r_image.astype(np.float32)
#         r_image /= 255.0
#         h=r_image.shape[0]
#         w = image.shape[1]
...
def cutout(image, labels):
    ...
    # create random masks
    scales = [0.0625] * 4 + [0.03125] * 8  # image size fraction
    for s in scales:
