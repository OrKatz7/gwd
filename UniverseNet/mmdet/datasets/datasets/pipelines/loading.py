...
class LoadMosaicImageAndAnnotations(object):
    ...
    def _load_mosaic_image_and_annotations(self, results):
        ...
        mosaic_mixup = False
        mosaic_prob = np.random.random()
        c_i = np.random.randint(4)
        if mosaic_prob>0.5:
            ...
            result = self._load_image_annotations(results_c, indexes[0])
            result_image = result['img'].astype(np.float32)
            result_boxes = result['gt_bboxes']
        else:
            result = self._load_image_annotations(results_c, indexes[0])
            result_image = result['img'].astype(np.float32)
            result_boxes = result['gt_bboxes']
            
            result1 = self._load_image_annotations(results_c, indexes[1])
            result_image1 = result1['img'].astype(np.float32)
            result_boxes1 = result1['gt_bboxes']
            
            result_image = result_image/2 + result_image1/2
            result_boxes = np.concatenate([result_boxes,result_boxes1], 0)
